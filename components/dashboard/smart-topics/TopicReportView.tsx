...
export const TopicReportView: React.FC<TopicReportViewProps> = ({
  ...
  return (
    <div className="flex-1 flex flex-col bg-card text-card-foreground overflow-hidden">
      ...
      {!hideHeader && (
        <div className="px-8 h-16 flex items-center justify-between border-b border-border/80">
          <div className="flex items-center gap-3 text-xs font-bold text-muted-foreground uppercase tracking-wider">
            ...
            <ChevronRight className="h-3.5 w-3.5 text-muted-foreground/50" />
            <span className="text-card-foreground truncate max-w-[200px]">报告预览</span>
          </div>
          ...
          <div className="flex items-center gap-4">
            <div className="relative hidden md:block">
              ...
              <input 
                type="text" 
                placeholder="搜索专题..." 
                className="bg-muted/50 border-none rounded-xl pl-9 pr-4 py-2 text-sm text-card-foreground focus:outline-none placeholder:text-muted-foreground/70 w-48 focus:w-64 transition-all"
              />
            </div>
            <Button variant="ghost" size="icon" className="h-9 w-9 rounded-xl text-muted-foreground hover:bg-muted">
              <Bell className="h-4.5 w-4.5" />
            </Button>
            <div className="h-8 w-8 rounded-xl bg-muted border border-border overflow-hidden shadow-sm">
        ...
        <div className="max-w-7xl mx-auto px-8 py-10 space-y-10">
            ...
            <div className="flex items-start justify-between">
              <div className="space-y-6 flex-1 max-w-4xl">
                <div className="flex items-center gap-3">
                  ...
                  <span className="text-[11px] font-bold text-muted-foreground/70 uppercase tracking-tight">更新于 2小时前</span>
                </div>

                <h1 className="text-5xl font-extrabold text-card-foreground leading-[1.15] tracking-tight">
                  {topic.title || "未命名专题报告"}
                </h1>
              ...
              <div className="flex items-center gap-3">
                <Button variant="outline" className="bg-card border-border rounded-xl gap-2 h-11 px-5 font-bold shadow-sm" onClick={onShare}>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-[1fr_360px] gap-12 pt-4">
            ...
            <div className="space-y-10">
              <div className="bg-card border border-border/90 rounded-[32px] shadow-sm overflow-hidden">
                ...
                <div className="px-8 h-14 flex items-center justify-between border-b border-border bg-muted/50">
                  ...
                  <div className="flex items-center gap-4 text-muted-foreground/70">
                    ...
                    <Volume2 className="h-4 w-4 cursor-pointer hover:text-card-foreground transition-colors" />
                    <Maximize2 className="h-4 w-4 cursor-pointer hover:text-card-foreground transition-colors" />
                  </div>
                </div>

                {/* Markdown Body */}
                <div className="p-10 prose prose-slate max-w-none prose-headings:font-bold prose-h1:text-3xl prose-h2:text-2xl prose-p:text-card-foreground prose-p:leading-relaxed prose-strong:text-blue-600 prose-ul:list-disc prose-li:text-card-foreground">
                  {!topic.summary_markdown?.trim() ? (
                    <div className="space-y-12">
                      <section>
                        <h2 className="text-card-foreground border-b-0 mb-6">一、事件背景</h2>
                        <p>
                      ...
                      <section>
                        <h2 className="text-card-foreground mb-6">二、核心观点</h2>
                        <div className="bg-blue-50 border-l-4 border-blue-500 p-6 rounded-r-2xl space-y-3 shadow-sm">
                          ...
                          <p className="italic text-popover-foreground font-medium leading-relaxed">
                        ...
                        <ul className="mt-10 space-y-6 list-none pl-0">
                          <li className="flex items-start gap-4">
                            ...
                            <div>
                              <div className="font-bold text-card-foreground mb-1">生产本地化加速</div>
                          ...
                          <li className="flex items-start gap-4">
                            <div className="h-2 w-2 rounded-full bg-blue-500 mt-2.5 shrink-0 shadow-[0_0_8px_rgba(59,130,246,0.4)]" />
                            <div>
                              <div className="font-bold text-card-foreground mb-1">替代化学技术崛起</div>
              ...
              {/* Disclaimer */}
              <p className="text-[11px] font-medium text-muted-foreground/70 leading-relaxed max-w-3xl">
            ...
            <div className="space-y-8">
              ...
              <div className="space-y-5">
                <div className="flex items-center justify-between">
                  <h3 className="text-xs font-bold text-muted-foreground/70 uppercase tracking-widest">来源文章</h3>
                  <span className="text-[10px] font-bold text-muted-foreground/50 uppercase">发现 {members.length} 篇</span>
                </div>
                <div className="space-y-3">
                  {members.slice(0, 4).map((m) => (
                    <div key={m.noteId} className="flex gap-4 p-4 bg-card hover:bg-muted rounded-2xl border border-border/80 shadow-sm transition-all group cursor-pointer" onClick={() => onOpenNote(m.noteId)}>
                      <div className="h-16 w-16 rounded-xl bg-muted shrink-0 overflow-hidden border border-border">
                        {m.note.cover_image_url ? (
                          ...
                          <div className="h-full w-full flex items-center justify-center text-muted-foreground/50 font-bold uppercase">{m.note.site_name?.charAt(0)}</div>
                        )}
                      </div>
                      <div className="flex-1 min-w-0 py-0.5">
                        <h4 className="text-[13px] font-bold text-card-foreground line-clamp-2 leading-snug group-hover:text-blue-600 transition-colors">{m.note.title}</h4>
                        <div className="mt-1.5 flex items-center gap-2 text-[10px] font-bold text-muted-foreground/70 uppercase">
              ...
              <div className="space-y-5">
                ...
                <div className="aspect-[4/3] rounded-[32px] bg-card border border-border/80 shadow-sm overflow-hidden relative group p-6 flex flex-col justify-end">
                  ...
                  <div className="relative z-10 space-y-4">
                    <div className="text-[10px] font-bold text-muted-foreground/70 uppercase tracking-wider">簇密度：高 / 拓扑结构稳定</div>
                    <div className="flex flex-wrap gap-2">
                      {keywords.slice(0, 5).map(k => (
                        <span key={k} className="text-[10px] font-bold text-muted-foreground bg-muted border border-border/80 px-2.5 py-1 rounded-lg shadow-sm">#{k}</span>
                      ))}
                    </div>
                  </div>
                </div>
