...
export function AnimatedThemeSwitcher({ variant = "default" }: AnimatedThemeSwitcherProps) {
  ...
  if (!mounted) {
    return (
      <div className={variant === "compact" ? "w-[46px] h-[46px]" : variant === "small" ? "w-9 h-9" : "w-10 h-10"} />
    );
  }
  ...
  if (variant === "small") {
    return (
      <div ref={containerRef} className="relative">
        ...
        <motion.button
          onClick={() => setIsOpen(!isOpen)}
          className="w-9 h-9 rounded-lg flex items-center justify-center text-slate-600 dark:text-slate-400 hover:bg-slate-100 dark:hover:bg-slate-800 transition-all duration-200 ease-out"
          whileHover={{ scale: 1.05 }}
          whileTap={{ scale: 0.95 }}
          title={`当前主题: ${currentTheme.label}`}
        >
        ...
        <AnimatePresence>
          {isOpen && (
            ...
            >
              <div className="bg-white dark:bg-slate-900 rounded-xl border border-slate-200 dark:border-slate-700 shadow-xl overflow-hidden min-w-[140px]">
                {themes.map((item, index) => {
                  const isActive = theme === item.value;
                  const Icon = item.icon;
