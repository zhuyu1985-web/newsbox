...
1. **沉浸阅读 (Reader/Player)：** _默认_。清洗后的纯净正文或智能播放器。
2. **原始网页 (Web View)：** iframe 加载原始 URL，保留原站排版。
3. **AI 速览 (AI Brief)：** _[原“快照”重命名]_ AI 生成的结构化事实卡片（核心事实、人物、结果），极简排版。
4. **网页存档 (Web Archive)：** _[新增]_ 永久保存的网页快照（HTML/图片），用于证据留存，防止原链接失效。

### 3.3 右侧：工具箱 (Toolbar)

#### A. 阅读器样式 (Appearance) - _自适应_
...
- **复制内容：**
  ...
  - **复制为引用 (Copy as Citation)：** _[核心]_ 格式：`> 选中文本 [链接](URL) (访问于 日期)`。
  - `复制为 ASR 逐字稿` (视频模式特有)。
- **导出：**
...
### 6.1 通用模块：批注列表 (Annotations) —— _核心功能_
