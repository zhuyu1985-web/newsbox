...
继续使用 `/notes/[id]`。在 `components/reader/ReaderPageWrapper.tsx` 顶层根据 `note.content_type` 分流：
...
| `video_jobs.cos_url`, `transcoded_url` | 播放源 |
...
| `POST /api/notes/[id]/draft` | **新增**：`sendBeacon` 兜底草稿保存端点 |
